...
export const PendingList = ({ pendingBookings, onApproveBooking, onEditBooking, salesList }) => {
  ...
  const handleStartEdit = (booking) => {
    ...
    setEditData({
      ...
      roomPriceStr: formatCurrencyInput(booking.roomPrice),
      depositStr: formatCurrencyInput(booking.deposit),
            ...
            {editingId === b.id ? (
              <div className="bg-blue-50 p-4 rounded-2xl border border-blue-200 space-y-3">
                ...
                <div className="p-2.5 bg-white rounded-lg space-y-1.5 text-xs border border-blue-300">
                  ...
                  <div className="flex justify-between text-gray-600">
                    <span>Phí Dịch Vụ (350k x {editBedroomsNum} P.Ngủ x {editNights} đêm):</span>
                    <span className="font-semibold text-gray-900">{editServiceFee.toLocaleString()} VNĐ</span>
                  </div>
                  <div className="flex justify-between text-gray-800 pt-1 border-t border-blue-200">
                    <span>Tổng Tiền:</span>
                    <span className="font-bold text-gray-900">{editTotalPrice.toLocaleString()} VNĐ</span>
                  </div>
                  <div className="flex justify-between text-blue-700">
                    <span>Còn Phải Thu:</span>
                    <span className="font-bold text-blue-700">{editRemainingPrice.toLocaleString()} VNĐ</span>
                  </div>
                </div>
